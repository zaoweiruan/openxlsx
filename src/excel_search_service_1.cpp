...
// ==== 日志输出 ====
void log_matches(const std::vector<Match>& matches, const std::string& keyword,bool detail=FALSE,std::string colume="A") {
...
//    	ASSERT_MSG(excel_file.empty(),"字符串为空 %s",excel_file);
    while (g_Running) {
        ...
        if (!clip.empty() && clip != last) {
            ...
            log_matches(matches, clip,detail,colume_str);
        }
        Sleep(2000);
    }
}
...
int wmain(int argc, wchar_t* argv[]) {
        ...
        // ===== 调试模式 =====
        setup_utf8_console();
        std::wstring excelFile = L"20250304.xlsm"; // 调试用路径
        if (argc>2)
        	 {run_service_loop(excelFile,_wtoi(argv[2]));}
        	else
        {run_service_loop(excelFile,FALSE,L"A"); }// 直接运行主循环
        return 0;
    }
