...
// ==== 日志输出 ====
void log_matches(const std::vector<Match>& matches,std::string filename, const std::string& keyword,bool detail=FALSE,std::string colume="A") {
...
void run_service_loop(const std::wstring& excel_path,bool detail=FALSE,std::wstring colume=L"A") {
    std::string last;
    std::string excel_file=WideCharConvertMultiByte(excel_path,GetACP());
    std::string colume_str=WideCharConvertMultiByte(colume);
//    	ASSERT_MSG(excel_file.empty(),"字符串为空 %s",excel_file);
    while (g_Running) {
        std::string clip = get_clipboard_text();
        if (!clip.empty() && clip != last) {
            last = clip;
            auto matches = search_excel_all_sheets(excel_file, CharsetConvert(clip.c_str(),GetACP(),CP_UTF8),colume_str);
            log_matches(matches,excel_file,clip,detail,colume_str);
        }
        Sleep(2000);
    }
}
...
//	    	std::cerr << "开始启动循环。\n";
    ...
    g_WorkerThread = std::thread(run_service_loop, excelFile,TRUE,L"A");
}

int wmain(int argc, wchar_t* argv[]) {
	    setup_utf8_console();
    auto args = parse_args(argc, argv);
    std::wstring filename = args[L"filename"];
//    std::wstring keyword = args[L"keyword"];
    std::wstring column = args[L"column"]; // 默认 A 列
    std::wstring mode = args[L"mode"]; // 默认 A 列
    std::wstring outdetail = args[L"outdetail"]; // 默认 A 列
    	
   if (!mode.empty() && _wcsicmp(mode.c_str(),L"debug") == 0) {
        // ===== 调试模式 =====  
        run_service_loop(filename,_wtoi(outdetail.c_str()),column); 
        return 0;
    }
